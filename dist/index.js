...
const druid_client_js_1 = require("./druid-client.js");
/**
...
const druidClient = new druid_client_js_1.DruidClient(config);
/**
...
// Tool handlers
async function handleExecuteSqlQuery(request) {
...
async function handleGetDatasourceMetadata(request) {
    const { datasource } = request.params.arguments;
    const metadata = await druidClient.getDatasourceMetadata(datasource);
    return {
        content: [
            {
...
async function main() {
    ...
    try {
        ...
        server.setRequestHandler(types_js_1.ListToolsRequestSchema, listTools);
        server.setRequestHandler(types_js_1.CallToolRequestSchema, callTool);
        server.setRequestHandler(types_js_1.ListResourcesRequestSchema, listResources);
        server.setRequestHandler(types_js_1.ReadResourceRequestSchema, readResource);
